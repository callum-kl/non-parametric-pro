...
def init(
    ...
    return ReplicaGibbsState(position, logdensity, logdensity_grad)
...
def build_kernel(logdensity_fn: Callable) -> Callable:
    ...
    def kernel(
        ...
        state: ReplicaGibbsState,
        parameters: NamedTuple,
    ) -> tuple[ReplicaGibbsState, ULAInfo]:
        new_state = one_step(
            rng_key,
            state,
        ...
        new_state = ReplicaGibbsState(*new_state)
...
def parametric_ula(
    ...
    def init_fn(position: jax.Array, rng_key: PRNGKey | None = None) -> ReplicaGibbsState:
        del rng_key
        return init(position, parameters, logdensity_fn)

    def step_fn(rng_key: PRNGKey, state: ReplicaGibbsState) -> tuple[ReplicaGibbsState, ULAInfo]:
