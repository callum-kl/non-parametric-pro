...
CONVERGENCE_ITERS = {
    "vgp": {100: None, 250: None, 500: None},
    "pro": {100: None, 250: None, 500: None},
}
...
def plot_iterations(split_dir: Path, out: Path) -> None:
    ...
    vgp_colors, pro_colors = ramp(VGP_COLOR, len(MS)), ramp(PRO_COLOR, len(MS))
    vgp_tops, vgp_bottoms = [], []

    for m, vgp_color, pro_color in zip(MS, vgp_colors, pro_colors):
        vgp_runs = load_runs(split_dir, "vgp_noncollapsed", m)
        if vgp_runs:
        ...
        pro_runs = load_runs(split_dir, "inducing_pro_gp", m)
        if pro_runs:
            scores = stack([r["avg_score"] for r in pro_runs])
            x = np.arange(1, scores.shape[1] + 1)
            mean = plot_trace(ax_pro, x, scores, pro_color, f"m = {m}")
            mark_convergence(ax_pro, x, mean, CONVERGENCE_ITERS["pro"][m])
    ...
    ax_pro.set(title="Inducing PrO-GP", xlabel="Gibbs iteration", ylabel="Average training score")
    for ax in (ax_vgp, ax_pro):
...
def plot_time_per_iteration(split_dir: Path, out: Path) -> None:
    ...
    for prefix, color, label in [
        ("vgp_noncollapsed", VGP_COLOR, "VGP (batch 512)"),
        ("inducing_pro_gp", PRO_COLOR, "Inducing PrO-GP"),
    ]:
        ms, means, lows, highs = [], [], [], []
        for m in MS:
    ...
    ax.set_yscale("log")
    ax.set_xticks(MS)
    ax.set(xlabel="Number of inducing points m", ylabel="Seconds per iteration")
    fig.tight_layout()
    fig.savefig(out, bbox_inches="tight")
    fig.savefig(out.with_suffix(".png"), dpi=200, bbox_inches="tight")
    plt.close(fig)
